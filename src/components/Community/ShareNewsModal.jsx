...
const ShareNewsModal = ({ news, onClose }) => {
    const [content, setContent] = useState(`ตรวจสอบข่าวนี้: ${news.title}\n\nอ่านเพิ่มเติม: ${window.location.origin}/news/${news.id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await communityService.shareNews({
                newsId: news.id,
                content
            });
                    ...
                    <div className="sm:flex sm:items-start">
                        ...
                        <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 className="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                พูดคุยเกี่ยวกับข่าวนี้
                            </h3>
                            <div className="mt-2">
                                <p className="text-sm text-gray-500 mb-2">
                                    เริ่มหัวข้อสนทนาเกี่ยวกับ "{news.title}" ในชุมชน
                                </p>
                                <textarea
