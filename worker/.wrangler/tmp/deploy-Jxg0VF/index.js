      ...
      try {
        ...
        if (bookmarksMatch) {
          ...
          if (request.method === "GET") {
            const bookmarks = await firestore2.runQuery({
          ...
          if (request.method === "POST") {
            try {
              ...
              const bookmark = await firestore2.createDocument("bookmarks", {
                user_id: auth.userId,
                target_type,
          ...
          if (request.method === "DELETE" && bookmarksMatch[1]) {
            ...
            const bookmark = await firestore2.getDocument("bookmarks", bookmarkId);
            if (!bookmark) return notFound();
            if (bookmark.user_id !== auth.userId) {
              return json({ success: false, message: "Unauthorized" }, { status: 403 });
            }
            await firestore2.deleteDocument("bookmarks", bookmarkId);
        ...
        if (url.pathname.startsWith("/api/chat")) {
          ...
          if (url.pathname === "/api/chat/unread-count" && request.method === "GET") {
            const received = await firestore2.runQuery({ from: [{ collectionId: "messages" }], where: { fieldFilter: { field: { fieldPath: "receiver_id" }, op: "EQUAL", value: { stringValue: myId } } } });
          ...
          if (url.pathname === "/api/chat/inbox/conversations" && request.method === "GET") {
            const sent = await firestore2.runQuery({ from: [{ collectionId: "messages" }], where: { fieldFilter: { field: { fieldPath: "sender_id" }, op: "EQUAL", value: { stringValue: myId } } } });
            const received = await firestore2.runQuery({ from: [{ collectionId: "messages" }], where: { fieldFilter: { field: { fieldPath: "receiver_id" }, op: "EQUAL", value: { stringValue: myId } } } });
            ...
            const conversations = await Promise.all(Array.from(convsMap.entries()).map(async ([friendId, lastMessage]) => {
              const friendDoc = await firestore2.getDocument("users", friendId);
          ...
          if (url.pathname === "/api/chat/send" && request.method === "POST") {
            ...
            const newMsg = await firestore2.createDocument("messages", {
              sender_id: myId,
          ...
          if (url.pathname === "/api/chat/read" && request.method === "POST") {
            ...
            const received = await firestore2.runQuery({ from: [{ collectionId: "messages" }], where: { fieldFilter: { field: { fieldPath: "receiver_id" }, op: "EQUAL", value: { stringValue: myId } } } });
            const unreadFromFriend = received.filter((m) => m.sender_id === friendId && !m.is_read);
            for (const m of unreadFromFriend) {
              await firestore2.updateDocument("messages", m.id, { is_read: true });
            }
            return json({ success: true });
          }
          ...
          if (chatMatch && request.method === "GET") {
            ...
            const sent = await firestore2.runQuery({ from: [{ collectionId: "messages" }], where: { fieldFilter: { field: { fieldPath: "sender_id" }, op: "EQUAL", value: { stringValue: myId } } } });
            const received = await firestore2.runQuery({ from: [{ collectionId: "messages" }], where: { fieldFilter: { field: { fieldPath: "receiver_id" }, op: "EQUAL", value: { stringValue: myId } } } });
            const chatHistory = [...sent.filter((m) => m.receiver_id === friendId), ...received.filter((m) => m.sender_id === friendId)];
        ...
        if (url.pathname.startsWith("/api/notifications")) {
          ...
          if (url.pathname === "/api/notifications" && request.method === "GET") {
            const limitStr = url.searchParams.get("limit") || "50";
            const notifications = await firestore2.runQuery({
          ...
          if (url.pathname === "/api/notifications/read" && request.method === "POST") {
            const body = await readJson(request);
            if (body.id) {
              await firestore2.updateDocument("notifications", body.id, { is_read: true });
            } else {
              const notifications = await firestore2.runQuery({
                from: [{ collectionId: "notifications" }],
                where: { fieldFilter: { field: { fieldPath: "user_id" }, op: "EQUAL", value: { stringValue: myId } } }
              ...
              for (const n of unread) {
                await firestore2.updateDocument("notifications", n.id, { is_read: true });
              }
            }
            return json({ success: true });
          }
        ...
        if (userThreadsMatch && request.method === "GET") {
          ...
          const threads = await firestore2.runQuery({
            from: [{ collectionId: "threads" }],
            where: {
              fieldFilter: {
        ...
        if (threadIdMatch && request.method === "DELETE") {
          ...
          await firestore2.deleteDocument("threads", threadIdMatch[1]);
          return json({ success: true });
        }
        ...
        if (url.pathname === "/api/users/profile" && request.method === "GET") {
          const auth = await requireAuthUserId(request, env);
          if ("error" in auth) return auth.error;
          const user = await firestore2.getDocument("users", auth.userId);
        ...
        if (url.pathname === "/api/users/profile" && request.method === "PUT") {
          ...
          await firestore2.updateDocument("users", auth.userId, updates);
          const updatedUser = await firestore2.getDocument("users", auth.userId);
          return json({ success: true, data: sanitizeUser(updatedUser) });
        }
        if (url.pathname === "/api/users/profile" && request.method === "DELETE") {
          const auth = await requireAuthUserId(request, env);
          if ("error" in auth) return auth.error;
          await firestore2.deleteDocument("users", auth.userId);
